...
from gui.cases.BrainMetSRSCase.GTV import PyPlanScoringBrainQT
...
class MainDialog(QtGui.QMainWindow, PyPlanScoringBrainQT.Ui_MainWindow):
    ...
    def setup_case(self, file_path, case_name, ini_file_path, rs_dvh=''):
        ...
        self.calc_kernel.setup_case(rs_dvh, file_path, self.case_name)
            ...
            if self.folder_root:
                ...
                if dcm_files['rtdose']:
                    ...
                    if len(dcm_files['rtdose']) == 1:
                        for p in dcm_files['rtdose']:
                            print("PyPlanScoring has loaded: ", os.path.basename(p))

                    if len(dcm_files['rtdose']) > 1:
                        file_names = [os.path.basename(dcm_file) for dcm_file in dcm_files['rtdose']]
                        print("PyPlanScoring will add 3D doses from files: ", file_names)
