...
def test_1(test1_calc_data, constraints_data, grid=0.1, lim=3, save_data=False, ):
    """
    ...
    result_num.to_excel(os.path.join(dest, 'Test_1_result.xls'))
    test_table.to_excel(os.path.join(dest, 'test_1_table_paper.xls'))

    print(test_table)


def test_2(test2_calc_data, constraints_data, grid=0.1, lim=3, save_data=False, ):
    # DICOM FILES
    ...
    dest = r'C:\Users\vgalves\Dropbox\Plan_Competition_Project\tests\tests_validation\results'
    result_num.to_excel(os.path.join(dest, 'Test_2_result.xls'))
