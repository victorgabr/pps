$(function () {
    ...
    function validateReport() {
        ...
        if (report.rs_filename && report.ct_filename && report.rd_filename.length > 0 && report.roi_number >= 0) {
    ...
    $('#ct_upload_input').fileupload({
        url: '/api/v1/upload_ct/',
        done: function(e, response){
            var current_data = JSON.parse(response.result);
            report.ct_filename = current_data.ct_filename;
            validateReport();
        ...
        add: function(e, data) {
            ...
            data.submit();
            if (data.files[0].name.indexOf("CT") == -1) {
                $("#warning_box").append('<div class="alert alert-warning alert-dismissable ct-alert" role="alert">Warning: CT filename does not contain "CT" which may indicate a non-CT file.</div>');
            }
    ...
    $("#create_report").click(function (e){
        ...
        if (report.ct_filename && report.rd_filename && report.rs_filename && roi_number >= 0) {
